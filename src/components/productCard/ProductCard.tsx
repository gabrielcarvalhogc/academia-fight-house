...
function ProductCard({ imageSrc, title, size, code }: ProductCardProps) {
    return (
        <Card style={{ width: '18rem', backgroundColor: "var(--white)", border: "none" }} className="rounded-4">
            ...
            <Card.Body className="text-center d-flex flex-column justify-content-evenly align-items-center">
                <Card.Title style={{ fontFamily: "var(--font-text)" }}>{title}</Card.Title>
                {size && <Card.Text>{size}</Card.Text>}
                <div style={{ height: "1px", width: "90%", backgroundColor: "#000000", margin: "0 auto" }}></div>
                <Card.Text className="mt-2 fs-5">{code}</Card.Text>
            </Card.Body>
        </Card>
    );
}
